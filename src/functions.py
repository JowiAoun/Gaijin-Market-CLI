...
def get_open_orders() -> list[tuple]:
    ...
    for item in data['response']:
        try:
            open_orders.append((int(item['txId']), int(item['id']), int(item['pairId']), item['market'], item['type'], round(int(item['localPrice'])/10000, 2), int(item['amount']), item['time']))
        except:
...
def cancel_order(transact_id: int, order_id: int, pair_id: int) -> bool:
    ...
    headers = {'accept': 'application/json, text/javascript, */*; q=0.01', 'accept-language': 'en-CA,en;q=0.8', 'content-type': 'application/x-www-form-urlencoded; charset=UTF-8'}
...
def db_get_hash_by_id(asset_id: int):
    ...
    """
    DB_CURSOR.execute(queries[3].format(asset_id))
